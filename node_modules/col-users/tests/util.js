...
var assertMe = module.exports.assertMe = function(me, expectedMe) {
  ...
  if (expectedMe) {
    ...
    assert.strictEqual(me.course.canvas_course_id, expectedMe.course.canvas_course_id);
    ...
    assert.strictEqual(me.course.canvas.use_https, expectedMe.course.canvas.use_https);
 ...
 * @param  {User}               [expectedUser]      The user to which the provided user should be compared
 * @param  {Boolean}            [expectEveryone]    Whether all users in the course are expected to be returned
 * @throws {AssertionError}                         Error thrown when an assertion failed
 */
var assertUser = module.exports.assertUser = function(user, expectedUser, expectEveryone) {
  // Ensure that all expected properties are present
  assert.ok(user);
  assert.ok(user.id);
  ...
  assert.ok(!user.bookmarklet_token);
  ...
  if (expectedUser) {
    ...
    assert.strictEqual(user.canvas_course_role, expectedUser.canvas_course_role);
    assert.strictEqual(user.canvas_full_name, expectedUser.canvas_full_name);
    assert.strictEqual(user.points, expectedUser.points);
    assert.strictEqual(user.share_points, expectedUser.share_points);

    // Ensure that all optional properties are the same as the ones for the
    // expected user
    if (user.canvas_image || expectedUser.canvas_image) {
      assert.strictEqual(user.canvas_image, expectedUser.canvas_image);
    }
    if (user.last_activity || expectedUser.last_activity) {
      assert.strictEqual(user.last_activity, expectedUser.last_activity);
    }
  }
};
...
/**
 * Assert that the users for the current course and their points can be retrieved
 *
 * @param  {Client}             client              The REST client to make the request with
 * @param  {Course}             course              The Canvas course the user is launched in
 ...
 * @throws {AssertionError}                         Error thrown when an assertion failed
 */
var assertGetUsers = module.exports.assertGetUsers = function(client, course, expectedTotal, expectEveryone, callback) {
  client.users.getUsers(course, function(err, users) {
    assert.ok(!err);
    assert.ok(users);
    assert.strictEqual(users.length, expectedTotal);
    _.each(users, function(user) {
      assertUser(user, null, expectEveryone);
    });
...
/**
 * Assert that the users for the current course and their points can not be retrieved
 *
 * @param  {Client}             client              The REST client to make the request with
 * @param  {Course}             course              The Canvas course the user is launched in
 * @param  {Number}             code                The expected HTTP error code
 ...
 * @throws {AssertionError}                         Error thrown when an assertion failed
 */
var assertGetUsersFails = module.exports.assertGetUsersFails = function(client, course, code, callback) {
  client.users.getUsers(course, function(err, users) {
    assert.ok(err);
    assert.strictEqual(err.code, code);
    assert.ok(!users);
