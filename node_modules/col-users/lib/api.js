 ...
 */
var getAllUsers = module.exports.getAllUsers = function(ctx, callback) {
  // Get the users from the DB
  var options = {
    'where': {
      'course_id': ctx.course.id
...
var getLeaderboard = module.exports.getLeaderboard = function(ctx, callback) {
  // Only administrators and users that have opted into sharing their points with
  ...
  // Only administrators are able to see all users in the course. Non-administrators
...
var updateUsers = module.exports.updateUsers = function(userIds, update, callback) {
  // Parameter validation
  var validationSchema = Joi.object().keys({
    'userIds': Joi.array().min(1).items(Joi.number()),
  ...
  DB.User.update(update, options).complete(function(err) {
    if (err) {
      log.error({'err': err, 'users': userIds}, 'Failed to update on or more users');
