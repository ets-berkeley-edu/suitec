...
/**
 * Initialize the Canvas poller if it has been enabled. The poller will check
 * with the Canvas REST API whether there are any new submissions or discussion
 * entries and track them as activities
 *
 * @api private
 */
var initializeCanvasPoller = function() {
  var isEnabled = config.get('canvasPoller.enabled');
  var interval = config.get('canvasPoller.interval') || 60;
  if (isEnabled) {
    CanvasPoller.enable(interval);
  }
};
