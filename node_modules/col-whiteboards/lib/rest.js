...
Collabosphere.apiRouter.post('/whiteboards/:id', function(req, res) {
  var members = CollabosphereUtil.toArray(req.body.members);
  WhiteboardsAPI.editWhiteboard(req.ctx, req.params.id, req.body.title, req.body.members, function(err, updatedWhiteboard) {
    if (err) {
      return res.status(err.code).send(err.msg);
    }
...
Collabosphere.apiRouter.get('/whiteboards/:id/chat', function(req, res) {
  WhiteboardsAPI.getChatMessages(req.ctx, req.params.id, function(err, chatMessages) {
    if (err) {
      return res.status(err.code).send(err.msg);
    }
