    ...
    CategoriesAPI.getCategoriesById(ctx, opts.categories, function(err, categories) {
      ...
      AssetsAPI.createWhiteboard(ctx, whiteboard, title, opts, function(err, asset) {
        ...
        generateThumbnail(whiteboard, function(err, whiteboard) {
          ...
          log.debug('Got an updated whiteboard');
          ...
          AssetsAPI.getAsset(ctx, asset.id, {'incrementViews': false}, function(err, asset) {
            ...
            return AssetsAPI.updateAssetPreview(asset, {'thumbnailUrl': whiteboard.thumbnail_url});
          });
        });
      });
    });
    ...
    embdr.process(imagePath, {
      'thumbnails': {
        ...
        'complete': function(thumbnails) {
          ...
          log.debug({'whiteboard': whiteboard.id}, 'Created a thumbnail for an exported whiteboard');
          updateWhiteboardPreview(whiteboard, thumbnails[CollabosphereConstants.THUMBNAIL_SIZE].url, cleanupFile);
        }
      },
      'error': function(err) {
        log.error({
 ...
 * @param  {Object}         thumbnailUrl                    The updated thumbnail URL of the whiteboard
 * @param  {Function}       callback                        Standard callback function
 * @param  {Object}         callback.err                    An error that occurred, if any
 * @api private
 */
var updateWhiteboardPreview = function(whiteboard, thumbnailUrl, callback) {
  // Parameter validation
  var validationSchema = Joi.object().keys({
  ...
  whiteboard.updateAttributes({'thumbnail_url': thumbnailUrl}).complete(function(err) {
    if (err) {
      log.error({
        'err': err,
        'thumbnailUrl': thumbnailUrl,
