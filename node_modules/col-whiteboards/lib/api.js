...
var whiteboardDeleteActivity = function(socket, elements, callback) {
  ...
  // Get the whiteboard elements that are being deleted
  async.eachSeries(elements, function(element, done) {
    var options = {
      'where': {
        'whiteboard_id': socket.whiteboard.id,
    ...
    DB.WhiteboardElement.find(options).complete(function(err, whiteboardElement) {
      if (err) {
        log.error({'err': err, 'whiteboardId': socket.whiteboard.id, 'uid': element.uid}, 'Failed to retrieve a whiteboard element to delete');
        return done({'code': 500, 'msg': err.message});
      } else if (!whiteboardElement) {
        log.debug({'err': err, 'whiteboardId': socket.whiteboard.id, 'uid': element.uid}, 'A whiteboard element that is being deleted could not be found');
        return done({'code': 404, 'msg': 'A whiteboard element that is being deleted could not be found'});
      }
  ...
  UsersAPI.getUser(userId, function(err, user) {
    ...
    getBasicWhiteboard(socket.ctx, whiteboardId, function(err, whiteboard) {
      ...
      socket.on('deleteActivity', function(elements) {
        ...
        whiteboardDeleteActivity(socket, element);
      });
