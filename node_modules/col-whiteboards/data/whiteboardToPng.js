...
var init = function() {
  ...
  getWhiteboardElements(function(whiteboardElements) {
    ...
    generatePng(whiteboardElements, function(pngStream) {

      // Pipe the PNG stream to standard out
      pngStream.pipe(process.stdout);

      // Close the process once we've sent the entire stream to standard out
 ...
 * @param  {Function}     callback                Standard callback function
 * @param  {Stream}       callback.stream         The stream of the PNG version of the elements on the canvas
 * @api private
 */
var generatePng = function(whiteboardElements, callback) {
  ...
  var render = _.after(whiteboardElements.length, function() {
    ...
    // Convert the canvas to a PNG file and return the stream
...
/**
 * Get a Fabric.JS canvas object. The `HelveticaNeue-Light` font will be pre-loaded
 *
