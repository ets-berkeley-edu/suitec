 ...
 * @param  {Category}           [expectedCategory]      The category to which the provided category should be compared
 * @param  {Boolean}            [expectAssetCount]      Whether the number of assets associated to the category are expected to be included
 * @throws {AssertionError}                             Error thrown when an assertion failed
 */
var assertCategory = module.exports.assertCategory = function(category, expectedCategory, expectAssetCount) {
  ...
  if (expectedCategory) {
    assert.strictEqual(category.id, expectedCategory.id);
    assert.strictEqual(category.course_id, expectedCategory.course_id);
    assert.strictEqual(category.title, expectedCategory.title);
    assert.strictEqual(category.created_at, expectedCategory.created_at);
    assert.strictEqual(category.updated_at, expectedCategory.updated_at);

    if (expectAssetCount) {
      assert.strictEqual(category.asset_count, expectedCategory.asset_count);
    }

    // Ensure that all optional properties are the same as the ones for the
    // expected category
    if (category.canvas_assignment_id || expectedCategory.canvas_assignment_id) {
      assert.strictEqual(category.canvas_assignment_id, expectedCategory.canvas_assignment_id);
    }
  }
};
...
var assertCreateCategory = module.exports.assertCreateCategory = function(client, course, title, callback) {
  client.categories.createCategory(course, title, function(err, category) {
    assert.ok(!err);
    assert.ok(category);
    assertCategory(category, null, true);
 ...
 * @throws {AssertionError}                                     Error thrown when an assertion failed
 */
var assertGetCategories = module.exports.assertGetCategories = function(client, course, expectedTotal, callback) {
  client.categories.getCategories(course, function(err, categories) {
    assert.ok(!err);
 ...
 * @throws {AssertionError}                                     Error thrown when an assertion failed
 */
var assertGetCategoriesFails = module.exports.assertGetCategoriesFails = function(client, course, code, callback) {
  client.categories.getCategories(course, function(err, categories) {
    assert.ok(err);
    assert.strictEqual(err.code, code);
    assert.ok(!categories);
 ...
 * @throws {AssertionError}                                     Error thrown when an assertion failed
 */
var assertEditCategory = module.exports.assertEditCategory = function(client, course, id, title, callback) {
  client.categories.editCategory(course, id, title, function(err, category) {
    assert.ok(!err);
    assert.ok(category);
    assertCategory(category, null, true);
 ...
 * @throws {AssertionError}                                     Error thrown when an assertion failed
 */
var assertEditCategoryFails = module.exports.assertEditCategoryFails = function(client, course, id, title, code, callback) {
  client.categories.editCategory(course, id, title, function(err, category) {
    assert.ok(err);
    assert.strictEqual(err.code, code);
    assert.ok(!category);
...
var assertDeleteCategory = module.exports.assertDeleteCategory = function(client, course, id, callback) {
  client.categories.deleteCategory(course, id, function(err) {
    ...
    client.categories.getCategories(course, function(err, categories) {
      assert.ok(!err);
      assert.ok(categories);
      assert.ok(!_.findWhere(categories, {'id': id}));
