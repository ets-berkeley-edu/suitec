...
var getCategories = module.exports.getCategories = function(ctx, callback) {
  ...
  var options = {
    ...
    'attributes': ['category.*', 'asset.id', [Sequelize.fn('COUNT', Sequelize.col('asset.id')), 'asset_count']],
    'group': [Sequelize.col('asset.id')],
    'include': [{'model': DB.Asset, 'attributes': ['id']}]
  };
 ...
 * @param  {Function}       callback                    Standard callback function
 * @param  {Object}         callback.err                An error that occurred, if any
 * @param  {Category[]}     callback.categories         The retrieved categorie
 */
var getCategoriesById = module.exports.getCategoriesById = function(ctx, ids, callback) {
  ...
  var validationSchema = Joi.object().keys({
    'ids': Joi.array().unique().items(Joi.number())
  });
...
var editCategory = module.exports.editCategory = function(ctx, id, title, callback) {
  ...
  // Get the category that is being edited
  var options = {
    'where': {
      'course_id': ctx.course.id,
      'id': id
  ...
  DB.Category.find(options).complete(function(err, category) {
    if (err) {
      log.error({'err': err, 'id': id}, 'Failed to retrieve the category to update');
      return callback({'code': 500, 'msg': err.message});
    } else if (!category) {
      log.debug({'err': err, 'id': id}, 'The category that is being edited could not be found');
      return callback({'code': 404, 'msg': 'The category that is being edited could not be found'});
    }
