...
describe('Assets', function() {
  ...
  describe('Like', function() {
    ...
    /**
     * Test that verifies that liking an asset contributes points to the engagement index
     */
    it('contributes points to the egagement index', function(callback) {
      // Create a few users in a course
      var adminUser = TestsUtil.generateAdminUser();
      TestsUtil.getAssetLibraryClient(null, null, adminUser, function(adminClient, course, adminUser) {
        TestsUtil.getAssetLibraryClient(null, course, null, function(client1, course, user1) {
          TestsUtil.getAssetLibraryClient(null, course, null, function(client2, course, user2) {
            TestsUtil.getAssetLibraryClient(null, course, null, function(client3, course, user3) {

              // Create an asset that can be liked
              AssetsTestUtil.assertCreateLink(client1, course, 'UC Davis', 'http://www.ucdavis.edu/', null, function(asset) {

                // Like the asset by two different users
                AssetsTestUtil.assertLike(client2, course, asset.id, true, function() {
                  AssetsTestUtil.assertLike(client3, course, asset.id, true, function() {

                    UsersTestUtil.assertGetLeaderboard(adminClient, course, 4, true, function(users) {
                      // The first user should have 7 points (5 for creating an asset and 2 for getting 2 likes)
                      assert.strictEqual(_.find(users, {'canvas_user_id': user1.id}).points, 7);
                      assert.strictEqual(_.find(users, {'canvas_user_id': user2.id}).points, 1);
                      assert.strictEqual(_.find(users, {'canvas_user_id': user3.id}).points, 1);
                      return callback();
                    });
                  });
                });
