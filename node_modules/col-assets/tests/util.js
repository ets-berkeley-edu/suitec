...
/**
 * Mock all the requests that are involved when uploading a file to Canvas
 *
 * @param  {Course}   course    The course to which the file will be uploaded
 * @api private
 */
var mockFileUpload = function(course) {
  var appServer = TestsUtil.getCanvasAppServer(course.canvas);
  ...
  var askUrl = util.format('/api/v1/courses/%d/files', course.id);
  ...
  var askRequest = new MockedRequest('POST', askUrl, null, null, askHandler, askValidator);
  ...
  var uploadRequest = new MockedRequest('POST', '/upload', null, null, redirectHandler);
