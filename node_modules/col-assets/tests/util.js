 ...
 * @param  {Boolean}            [expectedComments]      Whether the comments on the asset are expected to be included
 * @param  {Boolean}            [expectedCategories]    Whether the categories on the asset are expected to be included
 ...
 */
var assertAsset = module.exports.assertAsset = function(asset, expectedAsset, expectedCommentCount, expectedComments, expectedCategories, incrementViews) {
  ...
  if (_.isFinite(expectedCommentCount)) {
    assert.strictEqual(asset.comment_count, expectedCommentCount);
  }
  ...
  if (expectedAsset) {
    ...
    assert.strictEqual(asset.user.id, expectedAsset.user.id);
    assert.strictEqual(asset.user.canvas_course_role, expectedAsset.user.canvas_course_role);
    assert.strictEqual(asset.user.canvas_full_name, expectedAsset.user.canvas_full_name);
    ...
    if (expectedCategories) {
      assert.strictEqual(asset.categories.length, expectedAsset.categories.length);
      _.each(asset.categories, function(category) {
        var correspondingCategory = _.findWhere(expectedAsset.categories, {'id': category.id});
        CategoriesTestUtil.assertCategory(correspondingCategory);
        CategoriesTestUtil.assertCategory(category, correspondingCategory);
      });
    }
    ...
    if (asset.user.canvas_image || expectedAsset.user.canvas_image) {
      assert.strictEqual(asset.user.canvas_image, expectedAsset.user.canvas_image);
    }
  }
...
var assertCreateLink = module.exports.assertCreateLink = function(client, course, title, url, opts, callback) {
  ...
  client.assets.createLink(course, title, url, opts, function(err, asset, response) {
    assert.ok(!err);
    assert.ok(asset);
    assertAsset(asset, null, 0, true, true);
...
var assertGetAsset = module.exports.assertGetAsset = function(client, course, id, expectedAsset, expectedCommentCount, callback) {
  client.assets.getAsset(course, id, null, function(err, asset) {
    ...
    assertAsset(asset, expectedAsset, expectedCommentCount, true, true, true);
