    ...
    _.each(assets.results, function(asset) {
      ...
      if (filters.keywords) {
        ...
        _.each(keywords, function(keyword) {
          ...
          var descriptionContainsKeyword = (asset.description.toLowerCase().indexOf(keyword) !== -1);
          assert.ok(titleContainsKeyword || descriptionContainsKeyword);
        });
      }
      if (filters.category) {
        assert.ok(_.find(asset.categories, {'id': filters.category}));
      }
      if (filters.user) {
        assert.strictEqual(asset.user_id, filters.user);
      }
      if (filters.type) {
        assert.strictEqual(asset.type, filters.type);
      }
    });
