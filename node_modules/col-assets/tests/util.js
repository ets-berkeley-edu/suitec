...
var assertAsset = module.exports.assertAsset = function(asset, opts) {
  ...
  assert.ok(asset.user);
  assert.ok(asset.user.id);
  assert.strictEqual(asset.user.id, asset.user_id);
  assert.ok(asset.user.canvas_course_role);
  assert.ok(asset.user.canvas_full_name);
  assert.ok(!asset.user.bookmarklet_token);
  assert.ok(!asset.user.points);
  assert.ok(!asset.user.share_points);
  ...
  if (opts.expectedAsset) {
    ...
    assert.strictEqual(asset.user_id, opts.expectedAsset.user_id);
    ...
    assert.strictEqual(asset.user.id, opts.expectedAsset.user.id);
    assert.strictEqual(asset.user.canvas_course_role, opts.expectedAsset.user.canvas_course_role);
    assert.strictEqual(asset.user.canvas_full_name, opts.expectedAsset.user.canvas_full_name);
    ...
    if (opts.expectCategories) {
      ...
      _.each(asset.categories, function(category) {
        var correspondingCategory = _.findWhere(opts.expectedAsset.categories, {'id': category.id});
    ...
    if (asset.user.canvas_image || opts.expectedAsset.user.canvas_image) {
      assert.strictEqual(asset.user.canvas_image, opts.expectedAsset.user.canvas_image);
    }
  }
  ...
  if (asset.type === 'thought') {
    ...
    if (opts.expectedAsset) {
      assert.strictEqual(asset.body, opts.expectedAsset.body);
    }
  }
};
...
var assertCreateLink = module.exports.assertCreateLink = function(client, course, title, url, opts, callback) {
  ...
  client.assets.createLink(course, title, url, opts, function(err, asset) {
    ...
    if (opts.categories) {
      ...
      _.each(asset.categories, function(category) {
        assert.ok(_.contains(categories, category.id));
      });
    }
...
var assertGetAsset = module.exports.assertGetAsset = function(client, course, id, expectedAsset, expectedCommentCount, callback) {
  client.assets.getAsset(course, id, null, function(err, asset) {
    ...
    assertAsset(asset, {'expectedAsset': expectedAsset, 'expectedCommentCount': expectedCommentCount, 'expectComments': true, 'expectCategories': true, 'incrementViews': true});
 ...
 * @param  {String}             [options.keywords]              A string to filter the assets by
 * @param  {Number}             [options.category]              The id of the category to filter the assets by
 * @param  {Number}             [options.user]                  The id of the user who created the assets
 * @param  {Number}             [options.type]                  The type of assets
 * @param  {Number}             [options.limit]                 The maximum number of results to retrieve. Defaults to 10
 * @param  {Number}             [options.offset]                The number to start paging from. Defaults to 0
 ...
 * @throws {AssertionError}                                     Error thrown when an assertion failed
 */
var assertGetAssets = module.exports.assertGetAssets = function(client, course, options, expectedTotal, callback) {
  options = options || {};
  client.assets.getAssets(course, options, function(err, assets) {
    assert.ok(!err);
    assert.ok(assets);
 ...
 * @throws {AssertionError}                                     Error thrown when an assertion failed
 */
var assertGetAssetsFails = module.exports.assertGetAssetsFails = function(client, course, options, code, callback) {
  client.assets.getAssets(course, options, function(err, assets) {
    assert.ok(err);
    assert.strictEqual(err.code, code);
    assert.ok(!assets);
...
var assertEditAsset = module.exports.assertEditAsset = function(client, course, id, title, opts, callback) {
  ...
  client.assets.editAsset(course, id, title, opts, function(err, asset) {
    ...
    _.each(asset.categories, function(category) {
      assert.ok(_.contains(opts.categories, category.id));
    });
...
var assertDeleteComment = module.exports.assertDeleteComment = function(client, course, assetId, id, callback) {
  ...
  client.assets.getAsset(course, assetId, false, function(err, originalAsset) {
    assert.ok(!err);
    assertComment(_.findWhere(originalAsset.comments, {'id': id}));
    ...
    client.assets.deleteComment(course, assetId, id, function(err) {
      ...
      client.assets.getAsset(course, assetId, false, function(err, asset) {
        ...
        assert.ok(!_.findWhere(asset.comments, {'id': id}));
