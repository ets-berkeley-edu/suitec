...
Collabosphere.apiRouter.get('/assets/:assetId', function(req, res) {
  // Always increment the views when getting an asset, unless this
  // explicitly disabled
  ...
  AssetsAPI.assets.getAsset(req.ctx, req.params.assetId, incrementViews, function(err, asset) {
    if (err) {
...
Collabosphere.apiRouter.get('/assets', function(req, res) {
  AssetsAPI.assets.getAssets(req.ctx, req.query.limit, req.query.offset, function(err, assets) {
    if (err) {
      return res.status(err.code).send(err.msg);
    }
...
Collabosphere.apiRouter.post('/assets', function(req, res) {
  ...
  if (req.body.type === 'link') {
    ...
    AssetsAPI.assets.createLink(req.ctx, req.body.title, req.body.url, opts, function(err, createdLink) {
      if (err) {
        return res.status(err.code).send(err.msg);
      }
...
Collabosphere.apiRouter.post('/assets/:assetId/comments', function(req, res) {
  AssetsAPI.comments.createComment(req.ctx, req.params.assetId, req.body.body, req.body.parent, function(err, comment) {
    if (err) {
      return res.status(err.code).send(err.msg);
    }
...
Collabosphere.apiRouter.post('/assets/:assetId/comments/:commentId', function(req, res) {
  AssetsAPI.comments.editComment(req.ctx, req.params.assetId, req.params.commentId, req.body.body, function(err, comment) {
    if (err) {
      return res.status(err.code).send(err.msg);
    }
