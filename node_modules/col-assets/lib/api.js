...
var deleteComment = module.exports.deleteComment = function(ctx, assetId, id, callback) {
  ...
  var options = {
    ...
    'include': [
      {'model': DB.Asset, 'include': [{'model': DB.User, 'as': 'users'}]},
      {'model': DB.Comment, 'as': 'parent', 'required': false, 'include': [DB.User]},
      {'model': DB.User}]
  };
  DB.Comment.find(options).complete(function(err, comment) {
    if (err) {
      ...
      comment.destroy().complete(function(err) {
        ...
        comment.asset.decrement('comment_count').complete(function(err, asset) {
          ...
          deleteAssetCommentActivity(ctx, comment, function(err) {
            ...
            deleteGetAssetCommentActivities(ctx, comment, function(err) {
              if (err) {
                return callback(err);
              }

              return deleteGetAssetCommentReplyActivity(ctx, comment, callback);
            });
          });
        });
      });
 ...
 * @api private
 */
var deleteGetAssetCommentActivities = function(ctx, comment, callback) {
  var done = _.after(comment.asset.users.length, callback);
  var errorCallback = _.once(callback);

  // Delete the get_asset_comment activity for each asset owner
  _.each(comment.asset.users, function(assetUser) {
