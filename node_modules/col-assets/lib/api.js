 ...
 */
var getAssets = module.exports.getAssets = function(ctx, options, callback) {
  options = options || {};

  // Default some parameters
  options.limit = CollabosphereUtil.getNumberParam(options.limit, 10, 1, 25);
  options.offset = CollabosphereUtil.getNumberParam(options.offset, 0, 0);
  options.category = CollabosphereUtil.getNumberParam(options.category);
  options.user = CollabosphereUtil.getNumberParam(options.user);
  ...
  var validationResult = Joi.validate(options, validationSchema);
  if (validationResult.error) {
    return callback({'code': 400, 'msg': validationResult.error.details[0].message});
  }
  ...
  if (options.user) {
    where.$and.push({'user_id': options.user});
  }
  if (options.type) {
