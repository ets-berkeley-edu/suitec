...
var createCommentActivityGetAssetComment = function(ctx, asset, comment, parent, callback) {
  ...
  if (ctx.user.id !== asset.user.id || (parent && ctx.user.id !== parent.user.id)) {
    return ActivitiesAPI.createActivity(ctx.course, asset.user, 'get_asset_comment', comment.id, CollabosphereConstants.ACTIVITY.OBJECT_TYPES.COMMENT, null, ctx.user, callback);
  } else {
    return callback();
  }
};
...
var deleteComment = module.exports.deleteComment = function(ctx, assetId, id, callback) {
  ...
  var options = {
    ...
    'include': [
      {'model': DB.Asset, 'include': [DB.User]},
      {'model': DB.User}]
  };
    ...
    DB.Comment.findAll(options).complete(function(err, comments) {
      ...
      comment.destroy().complete(function(err) {
        ...
        comment.asset.decrement('comment_count').complete(function(err, asset) {
          ...
          ActivitiesAPI.deleteActivity(ctx.course, comment.user, 'asset_comment', comment.id, CollabosphereConstants.ACTIVITY.OBJECT_TYPES.COMMENT, null, function(err) {
            if (err) {
              return callback(err);
            }

            // Delete the get_asset_comment activity for the asset owner
            ActivitiesAPI.deleteActivity(ctx.course, comment.asset.user, 'get_asset_comment', comment.id, CollabosphereConstants.ACTIVITY.OBJECT_TYPES.COMMENT, comment.user, callback);
          });
        });
      });
    });
