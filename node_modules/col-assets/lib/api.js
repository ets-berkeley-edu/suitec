...
var createCommentActivityGetAssetComment = function(ctx, asset, comment, parent, callback) {
  ...
  if (ctx.user.id !== asset.user.id || (parent && ctx.user.id !== parent.user.id)) {
    return ActivitiesAPI.createActivity(ctx.course, asset.user, 'get_asset_comment', comment.id, CollabosphereConstants.ACTIVITY.OBJECT_TYPES.COMMENT, ctx.user, callback);
  } else {
    return callback();
  }
};
...
var createCommentActivityAssetComment = function(ctx, asset, comment, parent, callback) {
  ...
  if (ctx.user.id !== asset.user.id || (parent && ctx.user.id !== parent.user.id)) {
    return ActivitiesAPI.createActivity(ctx.course, ctx.user, 'asset_comment', comment.id, CollabosphereConstants.ACTIVITY.OBJECT_TYPES.COMMENT, null, callback);
  } else {
    return callback();
  }
};
...
var createCommentActivityGetAssetCommentReply = function(ctx, asset, comment, parent, callback) {
  ...
  if (parent && ctx.user.id !== parent.user.id) {
    return ActivitiesAPI.createActivity(ctx.course, parent.user, 'get_asset_comment_reply', parent.id, CollabosphereConstants.ACTIVITY.OBJECT_TYPES.COMMENT, ctx.user, callback);
  } else {
    return callback();
  }
};
