  ...
  embdr.process(assetUrl, {
    ...
    'thumbnails': {
      ...
      'complete': function(thumbnails) {
        updateAssetPreview(asset, {'thumbnailUrl': thumbnails[CollabosphereConstants.THUMBNAIL_SIZE].url});
      }
    },
    'images': {
      'sizes': [CollabosphereConstants.IMAGE_SIZE],
      'complete': function(images) {
        updateAssetPreview(asset, {'imageUrl': images[CollabosphereConstants.IMAGE_SIZE].url});
      }
    },
    'error': function(err) {
...
var createCommentActivityGetAssetComment = function(ctx, asset, comment, parent, callback) {
  ...
  if (!isAssetOwner || (parent && ctx.user.id !== parent.user.id)) {
    ...
    _.each(asset.users, function(assetUser) {
      ActivitiesAPI.createActivity(ctx.course, assetUser, 'get_asset_comment', comment.id, CollabosphereConstants.ACTIVITY.OBJECT_TYPES.COMMENT, null, ctx.user, done);
    });
  } else {
    return callback();
  }
        ...
        comment.asset.decrement('comment_count').complete(function(err, asset) {
          ...
          ActivitiesAPI.deleteActivity(ctx.course, comment.user, 'asset_comment', comment.id, CollabosphereConstants.ACTIVITY.OBJECT_TYPES.COMMENT, null, function(err) {
            ...
            deleteGetAssetCommentActivities(ctx, comment, function(err) {
              ...
              // Delete the get_asset_comment_reply activity for the parent commenter
