...
var getAssets = module.exports.getAssets = function(ctx, filters, limit, offset, callback) {
  ...
  var validationSchema = Joi.object().keys({
    'filters': Joi.object().keys({
      ...
      'type': Joi.string().valid(CollabosphereConstants.ASSET.ASSET_TYPES).optional(),
  ...
  if (filters.user) {
    where.$and.push({'user_id': filters.user});
  }
  if (filters.type) {
    ...
    asset.save().complete(function(err, asset) {
      ...
      asset.setCategories(categories).complete(function(err) {
        ...
        getAsset(ctx, asset.id, false, function(err, asset) {
          ...
          ActivitiesAPI.createActivity(ctx.course, ctx.user, 'add_asset', asset.id, CollabosphereConstants.ACTIVITY.OBJECT_TYPES.ASSET, null, null, function(err) {
            if (err) {
              return callback(err);
            }
...
var editAsset = module.exports.editAsset = function(ctx, id, title, opts, callback) {
  ...
  getAsset(ctx, id, false, function(err, asset) {
    if (err) {
      return callback(err);
    }
    ...
    // to edit it
    if (!ctx.user.is_admin && asset.user_id !== ctx.user.id) {
...
/**
 * Generate a thumbnail and embed code for an asset
 *
  ...
  embdr.process(assetUrl, {
    ...
    'thumbnails': {
      ...
      'complete': function(thumbnails) {
        updateAssetPreview(asset, {'thumbnailUrl': thumbnails['300x300'].url});
      }
    },
    'error': function(err) {
 ...
 * @api private
 */
var updateAssetPreview = function(asset, opts) {
  // Parameter validation
  var validationSchema = Joi.object().keys({
  ...
  getAsset(ctx, assetId, false, function(err, asset) {
    ...
    DB.Comment.create(comment).complete(function(err, comment) {
      ...
      asset.increment('comment_count').complete(function(err) {
        ...
        _createCommentActivities(ctx, asset, parentComment, function(err) {
          if (err) {
            return callback(err);
          }

          // Retrieve the created comment, including the associated user
          return getComment(comment.id, callback);
        });
      });
    });
  });
 ...
 *       - Alice gets points for a `create_comment` activity
 *       - Alice gets points for a `get_asset_comment` activity (maybe)
 ...
 * @param  {Number}         [parent]                        The parent comment to which the comment is a reply
 * @param  {Function}       callback                        Standard callback function
 * @param  {Object}         callback.err                    An error that occurred, if any
 * @api private
 */
var _createCommentActivities = function(ctx, asset, parent, callback) {
  _createCommentActivityGetAssetComment(ctx, asset, parent, function(err) {
    if (err) {
      return callback(err);
    }

    _createCommentActivityAssetComment(ctx, asset, parent, function(err) {
      if (err) {
        return callback(err);
      }

      return _createCommentActivityGetAssetCommentReply(ctx, asset, parent, callback);
    });
  });
};
 ...
 * @api private
 */
var _createCommentActivityGetAssetComment = function(ctx, asset, parent, callback) {
  ...
  if (ctx.user.id !== asset.user.id || (parent && ctx.user.id !== parent.user.id)) {
    return ActivitiesAPI.createActivity(ctx.course, asset.user, 'get_asset_comment', asset.id, CollabosphereConstants.ACTIVITY.OBJECT_TYPES.ASSET, ctx.user, callback);
  } else {
    return callback();
  }
};
 ...
 * @api private
 */
var _createCommentActivityAssetComment = function(ctx, asset, parent, callback) {
  // We create an `asset_comment` activity for the commenter if:
  //  - they are not the asset owner or
  //  - they are replying to another user
  if (ctx.user.id !== asset.user.id || (parent && ctx.user.id !== parent.user.id)) {
    return ActivitiesAPI.createActivity(ctx.course, ctx.user, 'asset_comment', asset.id, CollabosphereConstants.ACTIVITY.OBJECT_TYPES.ASSET, null, callback);
  } else {
    return callback();
  }
};
 ...
 * @api private
 */
var _createCommentActivityGetAssetCommentReply = function(ctx, asset, parent, callback) {
  ...
  if (parent && ctx.user.id !== parent.user.id) {
    return ActivitiesAPI.createActivity(ctx.course, parent.user, 'get_asset_comment_reply', asset.id, CollabosphereConstants.ACTIVITY.OBJECT_TYPES.ASSET, ctx.user, callback);
  } else {
    return callback();
  }
};
      ...
      ActivitiesAPI.deleteActivity(ctx.course, asset.user, ['get_like', 'get_dislike'], asset.id, CollabosphereConstants.ACTIVITY.OBJECT_TYPES.ASSET, ctx.user, function(err) {
        ...
        updateAssetLikeCount(asset, like, deletedLike, function(err, asset) {
          ...
          ActivitiesAPI.createActivity(ctx.course, ctx.user, type, asset.id, CollabosphereConstants.ACTIVITY.OBJECT_TYPES.ASSET, null, null, function(err) {
            ...
            ActivitiesAPI.createActivity(ctx.course, asset.user, type, asset.id, CollabosphereConstants.ACTIVITY.OBJECT_TYPES.ASSET, null, ctx.user, function(err) {
              if (err) {
                return callback(err);
              }
