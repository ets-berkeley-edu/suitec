...
var getAsset = module.exports.getAsset = function(ctx, id, incrementViews, callback) {
  ...
  var options = {
    ...
    'include': [{
      ...
      'include': [{
        'model': DB.User,
        'attributes': ['id', 'canvas_user_id', 'canvas_course_role', 'canvas_full_name', 'canvas_image']
      }]
    }]
  };
...
var getComment = function(id, callback) {
  ...
  DB.Comment.find(options).complete(function(err, comment) {
    if (err) {
      log.error({'err': err}, 'Failed to retrieve the created comment');
      return callback({'code': 500, 'msg': err.message});
    }
...
var editComment = module.exports.editComment = function(ctx, assetId, id, body, callback) {
  ...
  DB.Comment.find(options).complete(function(err, comment) {
    ...
    if ((!ctx.user.isAdmin() || comment.asset.course_id !== ctx.course.id) && comment.user_id !== ctx.user.id) {
