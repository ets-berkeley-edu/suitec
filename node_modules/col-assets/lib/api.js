...
var getAssets = module.exports.getAssets = function(ctx, filters, limit, offset, callback) {
  ...
  options.assignment = CollabosphereUtil.getNumberParam(options.assignment);
 ...
 * @param  {Number}         [opts.assignment]               The id of the assignment the asset if part of, if any
 ...
 * @param  {Boolean}        [opts.skipActivityCreation]     Whether creating an activity record should be created
 * @param  {Function}       callback                        Standard callback function
 * @param  {Object}         callback.err                    An error that occurred, if any
...
var createLink = module.exports.createLink = function(ctx, title, url, opts, callback) {
  ...
  var validationSchema = Joi.object().keys({
    ...
    'opts': Joi.object().keys({
      ...
      'skipActivityCreation': Joi.boolean().optional()
    })
  });
  ...
  // TODO: Remove this once proper thumbnail functionality is in place
  var thumbnail_url = '';
  var imageFormats = ['jpg', 'jpeg', 'png'];
  if (_.contains(imageFormats, url.toLowerCase().split('.').pop())) {
    thumbnail_url = url;
  }
  ...
  createAsset(ctx, asset, opts.categories, function(err, asset) {
    if (err) {
      return callback(err);
    }
    ...
    getAsset(ctx, asset.id, false, function(err, asset) {
      ...
      // If no activity needs to be created we can return early
      } else if (opts.skipActivityCreation) {
        return callback(null, asset);
      }
...
var createFile = module.exports.createFile = function(ctx, title, file, opts, callback) {
  ...
  CanvasAPI.uploadFileToCanvas(ctx, file.file, function(err, canvasItem) {
    ...
    // TODO: Remove this once proper thumbnail functionality is in place
    var thumbnail_url = '';
    if (canvasItem['content-type'].indexOf('image/') !== -1) {
      thumbnail_url = canvasItem.url;
    }
 ...
 *
 * @param  {Context}      ctx               Standard context containing the current user and the current course
 * @param  {Asset[]}      assets            The assets to delete
 * @param  {Function}     callback          Standard callback function
 */
var deleteAssets = module.exports.deleteAssets = function(ctx, assets, callback) {
  if (_.isEmpty(assets)) {
    return callback();
  }

  var assetIds = _.pluck(assets, 'id');

  // Before we destroy any assets, we fetch the activities that were related to them. That way
  // we can re-calculate the users who interacted with these assets their points
  DB.Activity.findAll({
    'where': {
      'asset_id': assetIds
    }
    ...
    // Destroy the assets, this will cascade down to the activities column
    // and remove any related activities
    DB.Asset.destroy({
      'where': {
        'id': assetIds
      }
      ...
      var userIds = _.chain(activities)
        ...
        .compact()
        .value();
      ActivitiesAPI.recalculatePoints(ctx.course, userIds, callback);
    });
  });
};
