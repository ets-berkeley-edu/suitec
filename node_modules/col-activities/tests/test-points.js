...
describe('Activity Points', function() {
  ...
  describe('Comments', function() {
    ...
    /**
     * Test that verifies that creating comments updates the points for the appropriate users
     */
    it('updates the points when commenting', function(callback) {
      // Bump the default test time-out as these tests are doing quite a few things
      this.timeout(5000);
    ...
    it('updates the points when deleting a comment', function(callback) {
      TestsUtil.getAssetLibraryClient(null, null, null, function(client1, course, user1) {
        TestsUtil.getAssetLibraryClient(null, course, null, function(client2, course, user2) {
          ...
          // User 1 creates an asset that user 2 comments on
          AssetsTestUtil.assertCreateLink(client1, course, 'UC Davis', 'http://www.ucdavis.edu/', null, function(asset) {
            AssetsTestUtil.assertCreateComment(client2, course, asset.id, 'Comment 1', null, function(comment) {

              // Verify deleting a comment updates the points accordingly
              ActivitiesTestUtil.assertDeleteCommentActivity(client2, client1, course, asset.id, comment.id, function() {
                return callback();
              });
            });
          });
