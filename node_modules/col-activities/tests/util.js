...
var assertEditActivityTypeConfiguration = module.exports.assertEditActivityTypeConfiguration = function(client, course, type, points, enabled, callback) {
  client.activities.editActivityTypeConfiguration(course, activityType, points, enabled, function(err, activityType) {
    assert.ok(!err);
    assert.ok(activityType);
    assertActivityType(activityType);
    if (!_.isNull(points)) {
      assert.strictEqual(activityType.points, points);
    }
    if (!_.isNull(enabled)) {
      assert.strictEqual(activityType.enabled, enabled);
    }

    return callback(activityType);
  });
};
