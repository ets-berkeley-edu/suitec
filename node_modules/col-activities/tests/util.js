 ...
 * @param  {RestClient}         [parentClient]                  The REST client representing the user that created the parent comment, if any
 * @param  {Course}             course                          The Canvas course in which the user is interacting with the API
 * @param  {Number}             assetId                         The id of the asset that is commented on
 * @param  {Number}             [parentId]                      The id of the parent comment, if any
 * @param  {Function}           callback                        Standard callback function
 * @throws {AssertionError}                                     Error thrown when an assertion failed
 */
var assertCreateCommentActivity = module.exports.assertCreateCommentActivity = function(commenterClient, creatorClient, parentClient, course, assetId, parentId, callback) {
  ...
  assertGetActivityTypeConfiguration(commenterClient, course, function(configuration) {
    var assetCommentPoints = _.findWhere(configuration, {'type': 'asset_comment'}).points;
    var getAssetCommentPoints = _.findWhere(configuration, {'type': 'get_asset_comment'}).points;
    var getAssetCommentReplyPoints = _.findWhere(configuration, {'type': 'get_asset_comment_reply'}).points;
    ...
    UsersTestUtil.assertGetMe(commenterClient, course, null, function(commenterMe) {
      UsersTestUtil.assertGetMe(creatorClient, course, null, function(creatorMe) {
        getParentCommentMe(parentClient, course, creatorMe, function(parentMe) {
          ...
          AssetsTestUtil.assertGetAsset(commenterClient, course, assetId, null, null, function(asset) {
            ...
            AssetsTestUtil.assertCreateComment(commenterClient, course, assetId, 'My special comment', parentId, function(comment) {
              ...
              if (commenterMe.id === creatorMe.id && !parentId) {
              ...
              } else if (commenterMe.id !== creatorMe.id && !parentId) {
                assertPoints(commenterClient, course, commenterMe, assetCommentPoints, true, function() {
                  assertPoints(creatorClient, course, creatorMe, getAssetCommentPoints, false, callback);
                });

              // A commenter who replies to their own comment on their own asset
              } else if (commenterMe.id === creatorMe.id && parentId && commenterMe.id === parentMe.id) {
                assertPoints(commenterClient, course, commenterMe, 0, false, callback);
              ...
              // A commenter who replies to a comment of someone else on their own asset
                ...
                assertPoints(commenterClient, course, commenterMe, assetCommentPoints + getAssetCommentPoints, true, function() {
                  assertPoints(parentClient, course, parentMe, getAssetCommentReplyPoints, false, callback);
                });
              ...
              // A commenter who replies to a comment of user on another user's asset where the asset
              // creator is the same user as the parent commenter
              } else if (commenterMe.id !== creatorMe.id && parentId && commenterMe.id !== parentMe.id && parentMe.id === creatorMe.id) {
                assertPoints(commenterClient, course, commenterMe, assetCommentPoints, true, function() {
...
/**
 * Get the me object for a client.
 * If no client was passed in, the `defaultMe` will be returned
 *
 * @param  {RestClient}     client          The client for which to retrieve the me feed
 * @param  {Course}         course          The Canvas course in which the user is interacting with the API
 * @param  {Object}         defaultMe       The default me object that should be returned if no client was passed in
 * @param  {Function}       callback        Standard callback function
 * @param  {Object}         callback.me     The me object for the client
 * @api private
 */
var getParentCommentMe = function(client, course, defaultMe, callback) {
  if (!client) {
    return callback(defaultMe);
  }

  return UsersTestUtil.assertGetMe(client, course, null, callback);
};
