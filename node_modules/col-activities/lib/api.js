 ...
 * @api private
 */
var getActivities = function(ctx, callback) {
  ...
  DB.Activity.findAll(options).complete(function(err, activities) {
    if (err) {
      log.error({'err': err, 'course': ctx.course.id}, 'Failed to get the activities');
...
var exportActivities = module.exports.exportActivities = function(ctx, callback) {
  ...
  getActivityTypeConfiguration(ctx.course.id, function(err, configuration) {
    ...
    getActivities(ctx, function(err, activities) {
      if (err) {
        return callback(err);
      }
      ...
      _.each(activities, function(activity) {
        ...
        formattedActivity.total = totalScores[activity.user_id];
