...
/**
 * Get a client that is launched into the Asset Library tool for a given Canvas course and user.
 ...
 * @param  {CanvasCourse}     [course]                The Canvas course the Asset Library will be launched in. Defaults to a new course in the `ucberkeley` Canvas instance
 * @param  {CanvasUser}       [user]                  The user in Canvas. Defaults to a new user in the `ucberkeley` Canvas instance
 * @param  {Function}         callback                Standard callback function
 * @param  {Client}           callback.client         The REST client that was used
 * @param  {CanvasCourse}     callback.course         The Canvas course that was used
 * @param  {CanvasUser}       callback.user           The Canvas user that was used
 * @throws {AssertionError}                           Error thrown when an assertion failed
 */
var getAssetLibraryClient = module.exports.getAssetLibraryClient = function(client, course, user, callback) {
  client = client || TestsUtil.getAnonymousClient();
  course = course || TestsUtil.generateCanvasCourse(global.tests.canvas.ucberkeley);
  user = user || TestsUtil.generateCanvasUser(global.tests.canvas.ucberkeley);
  assertAssetLibraryLaunchSucceeds(client, course, user, function() {
...
var assertAssetLibraryCartridgeSucceeds = module.exports.assertAssetLibraryCartridgeSucceeds = function(client, callback) {
  client.lti.assetLibraryCartridge(function(err, body, response) {
    ...
    xml2js.parseString(body, function(err, result) {
      ...
      assert.strictEqual(result.cartridge_basiclti_link['blti:description'][0], 'The asset library to display all submissions of all students');

      // Assert the correct launch URL is present
      assert.strictEqual(result.cartridge_basiclti_link['blti:launch_url'][0], '//localhost:2000/lti/assetlibrary');
      ...
      // Assert the correct name for in the Canvas menu is present
 ...
 * @return {Object[]}                         The matching property of `null` if no property could be found
 * @api private
 */
 ...
 *
 * @param  {Client}         client        The REST client to make the request with
 * @param  {CanvasCourse}   course        The Canvas course the Asset Library will be launched in
 * @param  {CanvasUser}     user          The user in Canvas
 * @param  {Function}       callback      Standard callback function
 * @throws {AssertionError}               Error thrown when an assertion failed
 */
 ...
 *
 * @param  {CanvasCourse}   course        The Canvas course the tool will be launched in
 * @param  {CanvasUser}     user          The user in Canvas
