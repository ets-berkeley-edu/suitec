...
var poll = function() {
  ...
  CourseAPI.getCourses(function(err, courses) {
    ...
    async.eachSeries(courses, throttledCourseHandling, function(err) {
      if (err) {
        log.error({'err': err}, 'Failed to poll all courses');
      } else {
...
var getAllUsers = function(ctx, callback) {
  ...
  UsersAPI.getAllUsers(ctx, function(err, users) {
    if (err) {
...
var handleUser = function(ctx, users, canvasUser, callback) {
  ...
  if (!enrollment) {
    ...
    if (_.contains(_.values(CollabosphereConstants.ENROLLMENT_STATE.ACTIVE), enrollment.enrollment_state)) {
  ...
  if (user && user.canvas_course_role === courseRole && user.canvas_enrollment_state === enrollmentState && user.canvas_full_name === canvasUser.name) {
    return callback();
  }
...
var pollAssignments = function(ctx, users, callback) {
  CanvasAPI.getAssignments(ctx.course, function(err, assignments) {
    ...
    CategoriesAPI.getCategories(ctx, function(err, categories) {
      if (err) {
        return callback(err);
      }
...
var handleAssignmentSubmissions = function(ctx, users, assignment, callback) {
  ...
  getActivities(ctx, 'submit_assignment', assignment.id, objectType, function(err, activities) {
    ...
    CanvasAPI.getSubmissions(ctx.course, assignment, function(err, submissions) {
      ...
      async.eachSeries(submissions, submissionIterator, function(err) {
        // We set the `user` property of the context in the submission handling logic. We no longer
        // need this and clean this up
...
/**
 * Handle a specific submission of an assignment
 *
 * @param  {Context}    ctx                   Standard context containing the current user and the current course
...
var handleSubmission = function(ctx, users, assignment, activities, submission, callback) {
  ...
  getOrCreateActivity(ctx, activities, users, activity, function(err, activity, created) {
    ...
    ctx.user = users[submission.user_id];
    ...
    AssetsAPI.getAssets(ctx, filters, null, null, function(err, assets) {
      ...
      // Delete any assets the user previously submitted for this assignment
      AssetsAPI.deleteAssets(ctx, assets.results, function(err) {
        if (err) {
          log.error({
            'assets': _.pluck(assets, 'id'),
            'err': err
        ...
        if (submission.submission_type === 'online_url') {
          ...
          AssetsAPI.createLink(ctx, null, submission.url, opts, callback);
        ...
        // Create a file asset for each attachment on an "upload" submission
          ...
          async.eachSeries(submission.attachments, handleFilesSubmissionAttachmentIterator, function(err) {
            ...
            if (activity.metadata.attempt !== submission.attempt) {
              ...
              return ActivitiesAPI.updateActivity(activity, {'metadata': activity.metadata}, callback);
            } else {
              return callback();
            }
          });
 ...
 * @param  {Number}       activity.user           The Canvas id of the user
 * @param  {Number}       [activity.actor]        The Canvas id of the user
 * @param  {Function}     callback                Standard callback function
 * @param  {Object}       callback.err            An error that occurred, if any
